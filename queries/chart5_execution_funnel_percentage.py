"""Chart 5: Total Funnel Executions (Percentage) - Query and visualization."""
...
def build_query(filters: Dict[str, Any]) -> str:
    ...
    """
    # Import from chart4 to reuse the exact same query logic
    from queries import chart4_execution_funnel
    return chart4_execution_funnel.build_query(filters)
...
def create_visualization(df: pd.DataFrame) -> go.Figure:
    ...
    metrics = [
        ...
        ("Webform Add New Card", (row["funnels_with_add_new_card_webform"] / base_count) * 100),
    ...
    fig.update_layout(
        title="Chart 5: Total Funnel Executions (% from Purchase Click)",
