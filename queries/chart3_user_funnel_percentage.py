"""Chart 3: Distinct Users Funnel (Percentage) - Query and visualization."""
...
def build_query(filters: Dict[str, Any]) -> str:
    """
    Build SQL query for distinct users funnel with percentages.
    
    Returns count of distinct users at each funnel step as percentage of purchase_click users.
    """
    ...
    SELECT
      ...
      (SELECT COUNT(DISTINCT distinct_id) FROM client_events 
       WHERE mp_event_name = 'purchase_successful' AND payment_platform = 'stash') as purchase_successful_client_users,
...
def create_visualization(df: pd.DataFrame) -> go.Figure:
    """Create bar chart visualization for the funnel with percentages."""
    if df.empty:
        return go.Figure()
    ...
    # Get base count (purchase_click_users)
    base_count = row["purchase_click_users"]
    ...
    metrics = [
        ...
        ("Changed Selection", (row["pre_purchase_changed_selection_users"] / base_count) * 100),
        ("Stash Continue", (row["pre_purchase_stash_continue_users"] / base_count) * 100),
        ("Native Popup", (row["got_native_popup_users"] / base_count) * 100),
        ("Webform Impression", (row["impression_stash_webform_users"] / base_count) * 100),
        ("Webform Add New Card", (row["webform_add_new_card_users"] / base_count) * 100),
        ("Webform Pay Click", (row["clicked_pay_stash_webform_users"] / base_count) * 100),
        ("Webform purchase_successful", (row["purchase_successful_stash_webform_users"] / base_count) * 100),
        ("Validation Request", (row["purchase_validation_request_users"] / base_count) * 100),
        ("Validation Approval", (row["purchase_validation_approval_users"] / base_count) * 100),
        ("Client purchase_successful", (row["purchase_successful_client_users"] / base_count) * 100),
        ("Rewards Granted", (row["rewards_granted_users"] / base_count) * 100),
    ]
    ...
    fig.update_layout(
        title="Chart 3: Distinct Users Funnel (% from Purchase Click)",
        xaxis_title="Funnel Step",
        yaxis_title="Percentage (%)",
        height=500,
        xaxis_tickangle=-45,
        showlegend=False,
        yaxis=dict(range=[0, 110])  # Set range to 0-110% for better visualization
    )
